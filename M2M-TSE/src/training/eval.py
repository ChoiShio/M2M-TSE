...
from src.training.dataset import FSDSoundScapesDataset, tensorboard_add_metrics
...
def test_epoch(model: nn.Module, device: torch.device,
    ...
    with torch.no_grad():
        with tqdm(total=len(test_loader), desc='Test', ncols=130) as t:
            for batch_idx, (mixed, gt, 
                            label_vec, time_label_vec, 
                            azim_vec, time_azim_vec) in enumerate(test_loader):
                ...
                if writer is not None:
                    tensorboard_add_metrics(
                        writer, tag='Test', metrics=metrics_batch, label=label_vec,
                        step=epoch)
                    
                # Show current loss and metrics in the progress meter
                avg_metrics_batch = {k[:4]: '%.05f\n'%(sum(metrics_batch[k])/len(metrics_batch[k])) for k in metrics_batch.keys()}
