...
from src.training.dataset import FSDSoundScapesDataset as Dataset
from src.training.dataset import tensorboard_add_sample
...
def train_epoch(model: nn.Module, device: torch.device, optimizer: optim.Optimizer,
                ...
                writer: SummaryWriter = None, data_params = None, scaler = None) -> float:
    ...
    with tqdm(total=len(train_loader), desc='Train', ncols=130) as t:
        for batch_idx, (mixed, gt, 
                        label_vec, time_label_vec, 
                        azim_vec, time_azim_vec) in enumerate(train_loader):
            
            mixed = mixed.to(device)
            gt = gt.to(device)
            label_vec = label_vec.to(device)            
            time_label_vec = time_label_vec.to(device)
            azim_vec = azim_vec.to(device)
            time_azim_vec = time_azim_vec.to(device)
            ...
            with autocast():
                ...
                output, loss = model(mixed, gt, 
                                     label_vec, time_label_vec, 
                                     azim_vec, time_azim_vec)
            ...
            avg_metrics_batch = {k[:4]: '%.05f'%(sum(metrics_batch[k])/len(metrics_batch[k])) for k in metrics_batch.keys()}
            t.set_postfix(loss='%.05f'%loss.mean().item(), **avg_metrics_batch)
            t.update()
...
def train(network, args: argparse.Namespace):
    ...
    if use_cuda:
        ...
        device = torch.device("cuda") # 'cuda:%d' % device_ids[0]
    ...
    try:
        ...
        for epoch in range(args.start_epoch, args.epochs + 1):
            ...
            curr_train_metrics = train_epoch(model, device, optimizer,
                                             ...
                                             data_params=args.train_data, scaler=scaler)
...
if __name__ == '__main__':
    ...
    random_seed = 230; args.random_seed = random_seed
