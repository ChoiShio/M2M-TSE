...
import tempfile
import torch  # --> added
import torchaudio.transforms  # --> added
...
def generate_from_jams(jams_infile,
    ...
    # Generate audio
    soundscape_audio, event_audio_list, scale_factor, ref_db_change, angle_list = \
    ...
    return soundscape_audio, soundscape_jam, annotation_list, event_audio_list, angle_list
...
class Scaper(object):
    ...
    def _generate_audio(self,
        ...
        with _set_temp_logging_level(temp_logging_level):
            ...
            for i, e in enumerate(ann.data):
                if e.value['role'] == 'background':
                    ...
                    # and set_output_format (to have sox interpret the output correctly).
                    tfm.convert(
                        samplerate=self.sr,
                        n_channels=self.n_channels,
                        bitdepth=None
                    ...
                    with _close_temp_files(tmpfiles_internal):
                        ...
                        # soundfile.write(f'./noise_final.wav', event_audio, self.sr)
                        # import matplotlib.pyplot as plt
                        # plt.figure(2023)
                        # plt.plot(event_audio)
                        # plt.savefig(f'./noise_final.png')
                        # plt.close()
                    ...
                    with _close_temp_files(tmpfiles_internal):
                        ...
                        ### added ###
                        event_audio = np.swapaxes(event_audio, 0, 1)  # [channel(1), time]
                        event_audio = self._resample(event_audio, event_sr, self.sr)[0]  # [time, ]

                        if 'SCn' not in e.value['source_file']: # spatialized (single channel or multi-channel)
                            RIRs = np.load(
                                os.path.join('../dataset', e.value['RIRs_dir'] + '.npy'))  # [# src, # mic, length]
                            reverb_audio = []
                            for j in range(RIRs.shape[1]):
                                sig = signal.convolve(event_audio, RIRs[e.value['RIRs_src_idx']][j])
                                sig = sig[0:event_audio.shape[0]]
                                reverb_audio.append(sig)
                            reverb_audio = np.array(reverb_audio)  # [channel(4), time]
                            if self.n_channels == 1:
                                event_audio = reverb_audio[0]  # [time, ]
                                event_audio = event_audio.reshape(-1, self.n_channels)  # [time, channel(1)]
                            elif self.n_channels == 4:
                                event_audio = reverb_audio # [0:8:2]
                                event_audio = np.swapaxes(event_audio, 0, 1)  # [time, channel(4)]
                        else: # non-spatialized (dry, single channel)
                            event_audio = event_audio.reshape(-1, self.n_channels)  # [time, channel(1)]

                        # NOW compute LUFS
                        fg_lufs = get_integrated_lufs(event_audio, self.sr)

                        # Normalize to specified SNR with respect to background
                        ...
                        # Pad with silence before/after event to match the soundscape duration
                        ...
                        event_audio_list.append(event_audio[:duration_in_samples])
                        if 'SCn' not in e.value['source_file']: # spatialized (single channel or multi-channel)
                            angle_list.append((e.value['azimuth_angle'], e.value['elevation_angle']))
                        else: # non-spatialized (dry, single channel)
                            angle_list.append((0.0, 0.0))
                else:
                    raise ScaperError(
        ...
        return soundscape_audio, event_audio_list, scale_factor, ref_db_change, angle_list
